...
func setupPostgresContainer(t *testing.T, ctx context.Context) (testcontainers.Container, *sql.DB) {
	t.Helper()
...
func TestParameterizedQueriesIntegration(t *testing.T) {
	...
	container, db := setupPostgresContainer(t, ctx)
	defer func() {
...
func BenchmarkParameterizedVsInline(b *testing.B) {
	...
	ctx := context.Background()
	container, db := setupPostgresContainer(&testing.T{}, ctx)
	defer func() {
		if err := db.Close(); err != nil {
	...
	b.Run("Inline", func(b *testing.B) {
		...
		// #nosec G202 - This is a benchmark test, not production code
