...
func NewTypeProvider(schemas map[string]Schema) TypeProvider {
	return &typeProvider{Base: celprovider.Base{Schemas: schemas, Mapper: sparkTypeToCELExprType}}
}
...
func NewTypeProviderWithConnection(_ context.Context, db *sql.DB) (TypeProvider, error) {
	...
	return &typeProvider{
		Base: celprovider.Base{Schemas: make(map[string]Schema), Mapper: sparkTypeToCELExprType},
		db:   db,
	}, nil
}
