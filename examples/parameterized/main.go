...
func demonstrateSimpleParameterized(ctx context.Context, env *cel.Env, db *sql.DB) {
	fmt.Println("\n" + strings.Repeat("-", 80))
	...
	// Execute query
	query := "SELECT id, name, age FROM users WHERE " + result.SQL
...
func demonstrateComplexParameterized(ctx context.Context, env *cel.Env, db *sql.DB) {
	fmt.Println("\n" + strings.Repeat("-", 80))
	...
	// Execute query
	query := "SELECT id, name, department, salary FROM users WHERE " + result.SQL
	rows, err := db.Query(query, result.Parameters...)
	if err != nil {
		log.Fatalf("Failed to execute query: %v", err)
	}
...
func demonstratePreparedStatement(ctx context.Context, env *cel.Env, db *sql.DB) {
	fmt.Println("\n" + strings.Repeat("-", 80))
	...
	query := "SELECT COUNT(*) FROM users WHERE " + result.SQL
...
func demonstrateQueryPlanCaching(ctx context.Context, env *cel.Env, db *sql.DB) {
	fmt.Println("\n" + strings.Repeat("-", 80))
	...
	query := "SELECT id, name, salary FROM users WHERE " + result.SQL
...
func demonstrateComparison(ctx context.Context, env *cel.Env, db *sql.DB) {
	fmt.Println("\n" + strings.Repeat("-", 80))
	...
	query1 := "SELECT COUNT(*) FROM users WHERE " + nonParamSQL
	var count1 int
	if err := db.QueryRow(query1).Scan(&count1); err != nil {
		log.Fatalf("Failed to execute non-param query: %v", err)
	}

	query2 := "SELECT COUNT(*) FROM users WHERE " + paramResult.SQL
	var count2 int
